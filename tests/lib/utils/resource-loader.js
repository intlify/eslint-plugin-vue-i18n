...
const ResourceLoader = require('../../../lib/utils/resource-loader')

describe('ResourceLoader', () => {
  const testFilename = path.resolve(__dirname, '../../fixtures/utils/resource-loader/test.json')
  it('should be refresh with change file.', async () => {
    const bk = fs.readFileSync(testFilename, 'utf8')
    try {
      let count = 0
      const resource = new ResourceLoader(testFilename, (p) => {
        count++
