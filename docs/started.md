  ...
  ...vueI18n.configs.recommended,
  {
    ...
    settings: {
      'vue-i18n': {
        ...
        //   },
        //   {
        //     // 'keyPrefix' case (messages are nested under the `errors` key)
        //     pattern: './path/to/locales4/**/errors.{json,json5,yaml,yml}',
        //     localePattern: /^.*\/(?<locale>[A-Za-z0-9-_]+)\/.*\.(json5?|ya?ml)$/,
        //     localeKey: 'path',
        //     keyPrefix: 'errors'
        //   },
...
- `localeDir` ... You can specify a string or an object or an array.
  ...
  - Object option
    ...
    - `keyPrefix` (Optional, `string`) ... Specifies the prefix key path under which the messages of the matched files are nested. The contents of each matched file are treated as if they were located at this key path. This is useful when the localization messages of a single locale are split across multiple files that are merged under different keys. For example, with `keyPrefix: 'errors'`, the messages of the matched file are resolved at the `errors` key path (so a `required` key in the file is referenced as `errors.required` in your source code).
