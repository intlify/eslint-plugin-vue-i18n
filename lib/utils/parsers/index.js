...
const { parseForESLint: parseJsonForESLint, getStaticJSONValue } = require('eslint-plugin-jsonc')
const { parseForESLint: parseYamlForESLint, getStaticYAMLValue } = require('yaml-eslint-parser')
...
function parseInI18nBlock (context, i18nBlock, parseForESLint) {
  ...
  return {
    ...
    sourceString,
    getSourceCode () {
      return resourceSourceCode || (resourceSourceCode = new SourceCode(sourceCode.text, ast))
    },
