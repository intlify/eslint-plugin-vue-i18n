...
function getUsedKeysMap(targetLocaleMessage, jsonValue, usedkeys) {
  ...
  for (const key of [...usedkeys, ...collectLinkedKeys(jsonValue)]) {
    ...
    return {
      ...
      enterNode(node) {
        ...
        pathStack = {
          ...
          node,
          usedKeys:
            (pathStack.usedKeys && pathStack.usedKeys[keys.key]) || false,
          keyPath: keys.path
        }
    ...
    return createVisitor(usedKeys, {
      ...
      resolveKeysForNode(parentPath, node) {
        ...
        if (parent.type === 'YAMLPair') {
          const key =
            typeof parent.key.value === 'undefined'
      ...
      buildFixer(node) {
        return function* (fixer) {
          const removeNode =
            node.parent.type === 'YAMLPair' ? node.parent : node
